...
class MyGUI:
    ...
    def __init__(self, my_parent):
        ...
        self.label = tk.Label(self.my_container_1, text="Tkinter Demo")
        self.label.pack()

        # Create a button object and place it in a container.
        # Note that widgets have attributes.
        self.button1 = tk.Button(self.my_container_1)
        ...
        self.button3 = tk.Button(self.my_container_1, command=self.button3_click)
        self.button3["text"] = "OK 2"
        self.button3["background"] = "green"
        self.button3.pack(side=tk.LEFT)

        self.button4 = tk.Button(self.my_container_1, command=self.button4_click)
        self.button4.configure(text="Cancel 2", background="red")
        self.button4.pack(side=tk.LEFT)

        self.button5 = tk.Button(self.my_container_1, command=self.button5_click, text="LAMBDA DEMO")
        ...
        self.my_container_2 = tk.Frame(my_parent, padx="5m", pady="5m")
        self.my_container_2.pack()
        # Create a text entry box
        self.text_label = tk.Label(self.my_container_2, text="Enter text:")
        self.text_label.grid(row=0, column=0, sticky=tk.W)
        self.text_input = tk.Entry(self.my_container_2)
        # self.text_input.bind("<Enter>", self.text_input.enter)
        self.text_input.grid(row=0, column=1, sticky=tk.W + tk.E)
        self.text_button = tk.Button(self.my_container_2, text="Get text input", command=self.text_input_enter)
        self.text_button.grid(row=1, column=1, sticky=tk.E)

        self.v = tk.StringVar()
        self.text_result = tk.Label(self.my_container_2, text="", textvariable=self.v)
        self.text_result.grid(row=2, column=0, sticky=tk.W, columnspan=2)

        # New container
        self.slide_container = tk.Frame(my_parent, padx="5m", pady="5m")
        self.slide_container.pack()

        # Slider
        self.slider = tk.Scale(self.slide_container, from_=0, to=100, orient=tk.HORIZONTAL)
        self.slider.grid(row=0, column=0, sticky=tk.W)
        self.v2 = tk.IntVar()
        self.v2 = self.slider.get()
        self.slide_result = tk.Label(self.my_container_2, text="", textvariable=self.v2)
        self.slide_result.grid(row=1, column=0, sticky=tk.W)
